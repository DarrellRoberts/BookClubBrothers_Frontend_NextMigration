...
const BrothersProfile: React.FC<Props> = ({ user, readBooks, userData }) => {
  ...
  const userBookObj = useMemo(() => {
    ...
    const bookIdArray = userData?.map((user) =>
      findBook(
    ...
    return Object.entries(userBookObj)
  }, [userData, readBooks])
      ...
      <div className="flex w-1/2 flex-col pl-10 pt-5 max-sm:pl-[25px] max-sm:pt-0">
        <ul>
          ...
          {user?.userInfo?.favGenre?.length > 0 ? (
            user?.userInfo?.favGenre?.map((genre, idx) => (
              <li
                key={`${genre}${idx}`}
                className="list-disc max-xs:text-[0.75rem]"
              >
                {genre}
              </li>
            ))
